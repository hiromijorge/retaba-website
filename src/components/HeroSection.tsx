    ...
    <section className="relative min-h-screen bg-brand overflow-hidden pt-16 lg:pt-20">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 min-h-[calc(100vh-5rem)] flex flex-col justify-between pb-8">
        ...
        <div className="pt-8 lg:pt-12 grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          {/* Left text */}
          <div className="z-10">
            {/* Certification badges — like COMPRO cover top-left */}
            <motion.div
            ...
            {/* Headline — matches COMPRO's bold black-on-yellow title */}
            <motion.div
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.6, delay: 0.1 }}
            >
              <p className="text-ink/60 font-bold text-sm uppercase tracking-widest mb-3">
                Catering Proposal
              </p>
              <h1 className="text-5xl sm:text-6xl lg:text-7xl font-black text-ink leading-[1.0] tracking-tight mb-6">
                Catering<br />
                Berkualitas,<br />
                <span className="text-green">Bersertifikat</span><br />
                <span className="text-green">Halal.</span>
              </h1>
              <p className="text-ink/70 text-lg leading-relaxed max-w-md mb-8">
                Mitra catering terpercaya untuk perusahaan dan pabrik di Tangerang, Salatiga, dan Solo.
                Menu bervariasi setiap minggu, bahan segar berkualitas.
              </p>
            </motion.div>
            ...
            >
              ...
              <Link
                href="/menu"
                className="inline-flex items-center justify-center gap-2 border-2 border-ink/20 hover:border-green text-ink hover:text-green bg-white/60 hover:bg-white px-8 py-4 rounded-2xl font-extrabold text-base transition-all hover:-translate-y-1"
              >
                Lihat Menu
        ...
        >
          <div className="bg-white rounded-2xl shadow-lg border border-border px-6 py-5 grid grid-cols-2 sm:grid-cols-4 gap-6">
            {[
              ...
              <div key={stat.label} className={`text-center ${i < 3 ? 'border-r border-border last:border-0' : ''}`}>
                <div className="text-2xl font-black text-green">{stat.value}</div>
                <div className="text-muted text-xs mt-0.5 font-medium">{stat.label}</div>
              </div>
            ))}
          </div>
        </motion.div>
