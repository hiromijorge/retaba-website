...
// The track scrolls by -50%, so it must hold two identical halves. Two copies per half keeps
// the row dense on ultra-wide screens; only the first copy is exposed to screen readers.
const row = [...clients, ...clients, ...clients, ...clients];

const LogoCard = ({ client, decorative }: { client: typeof clients[0]; decorative: boolean }) => (
  <div className="group flex-shrink-0 mx-6 flex flex-col items-center gap-4 cursor-default" aria-hidden={decorative || undefined}>
    ...
    <p className="text-ink/60 text-xs font-semibold text-center leading-tight max-w-[160px] group-hover:text-ink/80 transition-colors">
      {client.shortName}
    </p>
  </div>
);
  ...
  return (
    <section ref={ref} className="py-20 bg-white border-y border-border/30">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div>
            <p className="text-ink/60 text-xs uppercase tracking-widest font-bold mb-2">Klien Aktif</p>
      ...
      >
        ...
        <div className="marquee-track py-4">
          {row.map((client, i) => (
            <LogoCard key={`r-${i}`} client={client} decorative={i >= clients.length} />
          ))}
        </div>
      </motion.div>
